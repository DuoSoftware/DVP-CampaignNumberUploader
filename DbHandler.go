...
func GetTrackingInfo(res http.ResponseWriter, req *http.Request) {
	defer func() {
		if r := recover(); r != nil {
...
func RemoveCompleteProcess(res http.ResponseWriter, req *http.Request) {
	defer func() {
		if r := recover(); r != nil {
...
func TrackNumberUpload(res http.ResponseWriter, req *http.Request) {
	defer func() {
		if r := recover(); r != nil {
...
func UploadContactsToCampaignAndAttachSchedule(res http.ResponseWriter, req *http.Request) {
	defer func() {
		if r := recover(); r != nil {
	...
	for i := 0; i < gophers; i++ {
		...
		// spin up a gopher
		go SaveToDb(i, contacts, u1, uploadData, gophers)
		j = j + page
	}
...
func SaveToDb(gopher_id int, contacts []string, uid uuid.UUID, data UploadData, gophers int) {
	defer func() {
		if r := recover(); r != nil {
...
func AssingExssitingNumbersToCampaign(res http.ResponseWriter, req *http.Request) {
	defer func() {
		if r := recover(); r != nil {
